...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  btnEdit.addEventListener("click", async () => {
    ...
    if (isEditing) {
      ...
      try {
        ...
        localStorage.setItem(
          "norkys_currentUser",
          JSON.stringify({ ...sesion, ...actualizado }),
        );
        document.querySelector(".user-summary h3").textContent =
          actualizado.nombres;

        if (typeof showNorkysToast === "function")
          showNorkysToast("Datos actualizados", "success");
        if (typeof updateHeaderAndCart === "function") updateHeaderAndCart();
      } catch (err) {
        console.error("[profile.js] guardar perfil:", err);
        if (typeof showNorkysToast === "function")
          showNorkysToast("Error al guardar cambios", "error");
      }
  ...
  async function renderPedidos() {
    ...
    try {
      ...
      if (historialContainer && pedidos.length > 1) {
        historialContainer.innerHTML = pedidos
          .map((p) => {
            const fc = p.fechaCreacion
              ? new Date(p.fechaCreacion).toLocaleString("es-PE", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                })
  ...
  window.deleteAddress = async (index) => {
    try {
      const res = await fetch(
        `${API_URL}/usuarios/${user.id}/direcciones/${index}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (typeof showNorkysToast === "function")
        showNorkysToast("Dirección eliminada", "success");
    } catch {
      if (typeof showNorkysToast === "function")
        showNorkysToast("Error al eliminar dirección", "error");
    }
  };
  ...
  btnSaveAddress?.addEventListener("click", async () => {
    ...
    if (!alias || !detalle) {
      if (typeof showNorkysToast === "function")
        showNorkysToast("Completa todos los campos", "error");
      return;
    }
    ...
    try {
      ...
      addressModal.classList.remove("active");
      if (typeof showNorkysToast === "function")
        showNorkysToast("Dirección agregada", "success");
    } catch {
      if (typeof showNorkysToast === "function")
        showNorkysToast("Error al agregar dirección", "error");
    }
  });
