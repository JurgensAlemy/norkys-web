...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const renderActiveAddress = (dirs) => {
    if (!user) {
      addressContainer.innerHTML =
        "<strong>Invitado</strong><p>Inicia sesión para usar tus direcciones.</p>";
  ...
  if (btnChangeAddress) {
    btnChangeAddress.addEventListener("click", async () => {
      ...
      if (dirsActuales.length === 0) {
        addressesList.innerHTML =
          '<p style="color:#aaa;text-align:center;">No tienes direcciones guardadas.</p>';
      } else {
        dirsActuales.forEach((dir) => {
          const item = document.createElement("div");
          item.style.cssText =
            "padding:15px;border:1px solid var(--border-color);border-radius:10px;cursor:pointer;transition:all .2s;";
          item.onmouseover = () =>
            (item.style.borderColor = "var(--norkys-red)");
          item.onmouseout = () =>
          ...
          item.addEventListener("click", () => {
            ...
            if (typeof showNorkysToast === "function")
              showNorkysToast("Dirección actualizada", "success");
          });
  ...
  document
    .getElementById("closeAddressSelModal")
    ?.addEventListener("click", () => addressModal.classList.remove("active"));
  ...
  if (btnCheckout) {
    btnCheckout.addEventListener("click", async () => {
      ...
      const cart = getNorkysCart();
      if (cart.length === 0)
        return showNorkysToast("El carrito está vacío", "error");
      if (!selectedAddress)
        return showNorkysToast("Agrega una dirección en tu perfil", "error");

      // ===== NUEVO: validar stock antes de pasar al checkout =====
      btnCheckout.disabled = true;
      const textoOriginal = btnCheckout.innerHTML;
      btnCheckout.innerHTML =
        '<i class="fa-solid fa-spinner fa-spin"></i> Verificando stock...';
      ...
      try {
        ...
        for (const item of cart) {
          ...
          if (!prod || !prod.activo) {
            showNorkysToast(
              `"${item.nombre}" ya no está disponible y fue quitado del carrito`,
              "error",
            );
          ...
          if ((prod.stock ?? 0) <= 0) {
            showNorkysToast(
              `"${item.nombre}" está agotado y fue quitado del carrito`,
              "error",
            );
            cartActualizado = cartActualizado.filter((i) => i.id !== item.id);
            hayProblema = true;
          ...
          if (item.cantidad > prod.stock) {
            showNorkysToast(
              `Solo quedan ${prod.stock} de "${item.nombre}", se ajustó la cantidad`,
              "error",
            );
        ...
        sessionStorage.setItem(
          "norkys_checkout_address",
          JSON.stringify(selectedAddress),
        );
        window.location.href = "checkout.html";
      } catch (err) {
        showNorkysToast(
          "No se pudo verificar el stock, intenta de nuevo",
          "error",
        );
        btnCheckout.disabled = false;
        btnCheckout.innerHTML = textoOriginal;
