...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      if (!terms) return showNorkysToast('Debes aceptar los Términos y Condiciones', 'error');
      if (celular.length !== 9) return showNorkysToast('El celular debe tener 9 dígitos', 'error');
      if (password !== confirm) return showNorkysToast('Las contraseñas no coinciden', 'error');

      try {
        await registrarNorkys({ nombres, apellidos, correo, celular, password });
