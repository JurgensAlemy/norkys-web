...
window.changeNorkysCategory = (categoryId, element) => {
  document
    .querySelectorAll(".sticky-container .category-pill")
    ...
    newBtn.addEventListener("click", () => {
      ...
      if (prod && qty > (prod.stock ?? 0)) {
        if (typeof showNorkysToast === "function")
          showNorkysToast(
            `Solo quedan ${prod.stock} unidades de ${title}`,
            "error",
          );
        return;
      }
      ...
      if (existing) {
        ...
        if (prod && existing.cantidad + qty > (prod.stock ?? 0)) {
          if (typeof showNorkysToast === "function")
            showNorkysToast(
              `Ya tienes el máximo disponible de ${title} en tu carrito`,
              "error",
            );
          return;
        }
      ...
      if (typeof showNorkysToast === "function")
        showNorkysToast(`${qty}x ${title} agregado al pedido`, "success");
    });
  }
});
