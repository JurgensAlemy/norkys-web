...
const setupFunctionalSearch = () => {
  document.querySelectorAll(".search-input").forEach((searchInput) => {
    ...
    let resultsDrop = searchContainer.querySelector(
...
const setupIndexFilters = async () => {
  ...
  favoritesGrid.innerHTML =
    '<p style="color:#aaa;padding:20px;">Cargando...</p>';
  ...
  const productosFinales = products
    .filter((p) => {
      if (categoriasMostradas.has(p.categoria)) return false;
      categoriasMostradas.add(p.categoria);
      return true;
    })
    .slice(0, 5);
